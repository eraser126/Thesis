...
\par 本文方法的整体架构如\autoref{fig:method-architecture}所示。整体流程由视频帧采样、视觉特征提取、帧级空间 token 压缩、STC 时空融合和 LLM 生成推理五个部分组成。与原始 VideoLLaMA2 相比，本文新增的核心模块是位于视觉编码器与 STC Connector 之间的 TokenPacker 压缩模块，其作用是在保持视频时间维结构不变的前提下，减少每一帧内部的空间视觉 token 数量。

\par 具体而言，视频输入首先经均匀采样得到固定长度的帧序列，再由 CLIP 视觉编码器逐帧提取视觉 token。随后，改造后的 TokenPacker 以帧为单位执行空间聚合，将高分辨率 patch token 压缩为更少数量的帧级 token。压缩后的视觉表示继续送入 VideoLLaMA2 原有 STC Connector 完成时空建模，并最终与文本 token 一同输入 LLM 生成回答。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=.9\linewidth,keepaspectratio]{figure/method/arch.png}
    \caption{\label{fig:method-architecture}本文方法的整体架构}
\end{figure}
...
\par 为了更清楚地说明视觉特征构建过程，本文将其拆分为四个连续的工程步骤，如\autoref{tab:method-feature-construction-flow}所示。

\begin{table}[!htbp]
    \centering
    \caption{\label{tab:method-feature-construction-flow}视频视觉特征构建过程}
    \begin{tabularx}{\textwidth}{c l X}
        \toprule
        步骤 & 张量形式 & 说明 \\
        \midrule
        视频采样 & $B\times T\times C\times H\times W$ & 将原始视频采样为固定帧数，并保持显式时间维。 \\
        帧级展开 & $(BT)\times C\times H\times W$ & 合并 batch 维与时间维，使 CLIP 视觉塔可以按图像方式逐帧编码。 \\
        视觉编码 & $(BT)\times N\times D$ & 每帧图像被划分为 $N$ 个 patch token，并映射到 $D$ 维视觉特征空间。 \\
        时间维恢复 & $B\times T\times N\times D$ & 将逐帧编码结果恢复为视频特征格式，供后续 TokenPacker 和 STC Connector 使用。 \\
        \bottomrule
    \end{tabularx}
\end{table}

\par 该流程的核心思想是：视觉编码阶段复用 CLIP 的图像级前向传播能力，而在编码完成后恢复视频的时间维组织。这样既保留了帧序列的时间顺序，又能够复用已有 CLIP 权重和 VideoLLaMA2 视觉塔实现。由于每帧都会产生 $N$ 个 patch token，原始视频在进入 STC Connector 前的视觉 token 总数为
...
\par 将 TokenPacker 迁移到 VideoLLaMA2 的关键工程问题在于维度对齐。原始 VideoLLaMA2 的 STC Connector 接收 $B\times T\times N\times D$ 的视觉特征，其中 $D$ 为视觉编码器 hidden size。压缩后，本文仅改变空间 token 数量，将 $N$ 替换为 $N_c$，而不改变时间维、batch 维和通道维。该维度变化如\autoref{tab:method-dimension-alignment}所示。

\begin{table}[!htbp]
    \centering
    \caption{\label{tab:method-dimension-alignment}TokenPacker 接入前后的维度对齐方式}
    \begin{tabularx}{\textwidth}{l c X}
        \toprule
        阶段 & 特征形状 & 说明 \\
        \midrule
        压缩前 & $B\times T\times N\times D$ & STC Connector 原始输入格式，每帧包含 $N$ 个空间 token。 \\
        TokenPacker 压缩 & $B\times T\times N_c\times D$ & 仅减少每帧空间 token 数量，保持时间维和通道维不变。 \\
        STC 输入适配 & $B\times T\times N_c\times D$ & 压缩后特征仍符合 STC 的帧级视频输入组织方式。 \\
        \bottomrule
    \end{tabularx}
\end{table}
